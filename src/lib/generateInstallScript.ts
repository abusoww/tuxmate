...
export function generateInstallScript(options: GenerateOptions): string {
  ...
  switch (distroId) {
    ...
    case "arch":
      return generateArchScript(packages, helper);
    case "fedora":
    ...
    case "void":
      return generateVoidScript(packages);
    case "flatpak":
      return generateFlatpakScript(packages);
    case "snap":
      return generateSnapScript(packages);
    case "homebrew":
      return generateHomebrewScript(packages);
    default:
...
export function generateCommandline(options: GenerateOptions): string {
  ...
  switch (distroId) {
    ...
    case "nix":
      return generateNixConfig(packages);
    case "void":
      return `sudo xbps-install -y ${pkgList}`;
    case "flatpak":
    ...
    case "homebrew": {
      ...
      if (casks.length > 0) {
        parts.push(
          `brew install --cask ${casks.map((p) => p.pkg.replace("--cask ", "")).join(" ")}`,
        );
      }
